...
app_server <- function(input, output, session) {
  ...
  grouped_data <- reactive({
    ...
    # Utilisation de `eval()` pour permettre un `group by` dynamique
    dt[, lapply(.SD, sum, na.rm = TRUE), by = eval(parse(text = paste(input$group_var, collapse = ", "))), .SDcols = sapply(dt, is.numeric)]
  })
